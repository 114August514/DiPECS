...
除了代码实现本身，DiPECS 还有一套比较完整的项目文档，包括需求分析、可行性报告、中期报告、结题报告、当前实现文档、系统设计文档、RFC、团队分工、环境配置、CI 说明、编码规范和会议纪要。这些文档的优点是覆盖面比较广：评审者既可以从 `current/overview.md` 了解当前可运行主链路，也可以从 `design/overview.md` 和 RFC 了解架构演化，还可以通过 `research/deliverables` 看到课程交付材料。
