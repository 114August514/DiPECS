...
| `evaluation/academic-next-app-baselines.json` | next-app 学术 baseline 对照表，区分 direct / contextual_only / excluded_unclear 可比性 | 手工维护，`academic_next_app_baselines_test` 校验 |
...
`evaluation/` 根目录下还包含 Cloud LLM 准确率评估相关的数据文件：

| 文件 | 说明 |
|------|------|
| `cloud-llm-accuracy-cases.json` | 手工编写的30条 seed 测试用例（schema v1），覆盖多种 persona 和场景 |
| `cloud-llm-accuracy-cases.battery.json` | 从电池/应用使用数据生成的用例（300 条） |
| `cloud-llm-accuracy-cases.lsapp.json` | 从 LSApp 生成的用例（1000 条，action-intent 模式） |
| `cloud-llm-accuracy-cases.lsapp-next-app.json` | 从 LSApp 生成的 next-app 预测用例（100 条） |
| `cloud-llm-accuracy-cases.lsapp-action-intent.json` | 从 LSApp 生成的 action-intent 用例（100 条） |
| `cloud-accuracy-topk-*.json` | Python 评估器（`tools/evaluate/evaluate_cloud_accuracy.py`）的运行结果，schema v2 |

`cloud-accuracy-topk-*.json` 文件的命名格式为 `cloud-accuracy-topk-<unix_timestamp>.json`，每次运行生成一个。关键字段：

- `environment.match_mode`：匹配模式（`full`=完整匹配 / `core`=intent+action+extension / `action`=仅 action_type）
- `results.top1_accuracy_pct`：Top-1 准确率
- `reference.top5_meets_reference`：是否达到90% Top-5 基准线
