...
按 DiPECS 真实管线裁剪文档的 18 态。**只保留当前可达的状态**，不引入任何不可达的死 variant（budget/调度类终态延后到机制存在时的后续 RFC）：
...
- **issue #5**：覆盖 schema/privacy/capability/policy/failed 五类终态 + 「恰好一条终态审计」+ 不 panic。**不覆盖**「预算拒绝有对应终态」（`ActionState` 不引入 `DeniedByBudget`）与 `Scheduled`/`BudgetReserved`/`Expired`/`Cancelled`（均无机制支撑）。→ 在 issue #5 勾掉已完成项，对未做项注明「延后到 budget/调度机制存在时的后续 RFC」，**该 issue 不由本 PR 完全关闭**。
- **issue #8**：全部验收项覆盖。**可关闭**。
