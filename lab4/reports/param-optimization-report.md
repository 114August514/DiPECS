...
| 配置 | Prompt (t/s) | Generation (t/s) |
| :--- | ---: | ---: |
| mmap=1 (默认) | **155.38** | 31.03 |
| mmap=0 | 153.21 | **31.30** |

**分析**：
- 两种模式的持续推理吞吐差异不足 1.5%，本轮不能认定某一方明显更快。
- `mmap` 的主要价值是建立文件到虚拟地址空间的映射，并配合按需缺页和页缓存降低独立进程的启动成本；它不保证稳态矩阵计算更快。
- 启动短请求实验中，`mmap` 平均 676.2 ms，`no-mmap` 平均 904.0 ms，因此交互式、频繁启动的工作负载仍建议保留默认 `mmap`。
