...
- 涉及的模块：
  ...
  - `apps/android-collector/`：继续作为 Phase 1 用户态采集与接口筛选工具，保留 app 侧 trace preview；生产结构化输出由 Rust 侧生成。
...
- 向后兼容性：
  - 现有 JSONL `rawEvent` 外部标签格式可作为 Phase 1 调试和 Rust 入口样本保留。
...
2. 将 `apps/android-collector` 的 JSONL trace 作为 Phase 1 输入验收材料，保留 `rawEvent` 作为 Rust 入口样本而非最终结构化输出。
...
- [架构概览](../overview.md)
- [设计哲学](../philosophy.md)
- [Daemon 架构设计](../daemon-architecture.md)
- [Android 接口最小可运行边界](../android-interface-mvp.md)
