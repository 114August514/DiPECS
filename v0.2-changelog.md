...
### 3.1 daemon 主循环 (`crates/aios-agent/src/main.rs`)

**变更**: 165 行 → 264 行 (重构, 后从 aios-adapter 迁移至 aios-agent — 见 3.3)
...
原主循环的采集逻辑完整迁移至独立的 async block (`crates/aios-agent/src/main.rs:76`)。行为不变：每 100ms 扫描 `/proc` 差分 + Binder eBPF 轮询，每 30s 采集系统状态。事件通过 `raw_events_tx.send()` 推送到处理管道。退出信号通过 `broadcast::Receiver::try_recv()` 非阻塞轮询。

#### 处理管道 (task 2)

运行在主 task 上 (`crates/aios-agent/src/main.rs:145`)。核心循环使用 `tokio::select!` 等待三路事件：
...
窗口关闭后调用 `process_window()` (`crates/aios-agent/src/main.rs:225`)，执行 `MockCloudProxy::evaluate() → PolicyEngine::evaluate_batch() → executor.execute_batch()` 完整链路。所有步骤均通过 `tracing::info!`/`tracing::warn!`/`tracing::debug!` 打点。
...
**问题**: v0.2 初始合并时，`aios-adapter` 直接依赖 `aios-agent` 以使用 `MockCloudProxy`，违反了 CLAUDE.md 规定的「上层依赖下层」单向层级：
...
| 文件 | 变更 | 说明 |
|:---|:---|:---|
| `crates/aios-adapter/Cargo.toml` | 移除 `aios-agent` 依赖 + `[[bin]]` 节 | adapter 回归纯库角色 |
| `crates/aios-agent/Cargo.toml` | 新增 `aios-adapter`, `aios-core`, `aios-kernel`, `tokio`, `tracing-subscriber`, `anyhow` 依赖 + `[[bin]]` 节 | agent 成为 daemon 宿主 |
| `crates/aios-agent/src/main.rs` | 新建 (从 `aios-adapter/src/main.rs` 迁移, 264 行) | daemon 二进制归属 agent |
| `crates/aios-adapter/src/main.rs` | 删除 | — |
| `CLAUDE.md` | 更新 daemon 二进制路径 → `crates/aios-agent/src/main.rs` | 文档与代码一致 |

修正后依赖方向正确: `aios-agent → aios-adapter → aios-kernel → aios-core → aios-spec`。

验证: `cargo tree -p aios-adapter | grep -E "aios-agent|aios-cli"` 无输出。
...
| `aios-kernel` workspace dep (新增) | `Cargo.toml` | kernel crate 进入 workspace |
| `aios-adapter`, `aios-core`, `aios-kernel` (新增) | `crates/aios-agent/Cargo.toml` | agent 依赖下层所有 crate (符合层级) |
| `tokio`, `tracing-subscriber`, `anyhow` (新增) | `crates/aios-agent/Cargo.toml` | daemon 二进制需要 |
| ~~`aios-agent`~~ (移除) | `crates/aios-adapter/Cargo.toml` | 修正反向依赖 (见 3.3 节) |
...
```
  ...
  crates/aios-agent/Cargo.toml        |   8 + (依赖 + [[bin]])
  crates/aios-agent/src/lib.rs        | 212 +++ (MockCloudProxy 重写)
  crates/aios-agent/src/main.rs       | 264 +++ (daemon 二进制, 从 adapter 迁移)
  ...
  crates/aios-adapter/Cargo.toml      |   1 - (移除 aios-agent 依赖 / [[bin]])
  crates/aios-adapter/src/main.rs     |   0 - (删除, 迁移至 aios-agent)
