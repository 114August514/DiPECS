...
> Last verified as history: 2026-06-30  
> 本页保留 v0.2 当时的实现记录，其中 `MockCloudProxy`、旧 `ActionExecutor`、`ActionResult`、`execute_batch` 等名称已经不代表当前源码。当前实现请看 [当前实现总览](../../architecture/index.md)。

> **日期**: 2026-05-05
> **基线**: v0.1 (aios-spec 宪法层 + aios-core 核心逻辑 + collector 采集骨架)
...
| `docs/src/architecture/crates.md` | 更新 daemon 二进制路径 → `crates/aios-daemon/src/main.rs` | 文档与代码一致 |
...
5. **交叉编译** (可选): `source scripts/dev/setup-env.sh && cargo android-release` → 编译成功
...
| 模块 | 状态 | 计划 |
|:---|:---|:---|
| `aios-agent` Cloud LLM | DeepSeek/Qwen/OpenAI-compatible 已接入 | 继续补充真实 provider 回归样本 |
| `aios-action` ActionExecutor | replay fallback + Android `PrefetchFile` bridge 已接入 | 后续收敛更多 Android-safe 自有资源动作 |
| `FanotifyMonitor` | 未实现 | `FsAccessEvent` 类型已有，采集器待写 |
| Android JSONL ingress | 已接入 | `dipecsd --android-trace-jsonl` 持续消费 append-only `rawEvent` JSONL |
| RuntimeTraceRecorder | 已接入 daemon 主循环 | `--trace-output` 输出窗口级 NDJSON |
