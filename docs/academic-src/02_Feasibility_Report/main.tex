...
DiPECS 是一个面向 Android/Linux 端侧环境的意图预测与动作治理原型。
本报告从数据采集、系统架构、决策模型、动作执行、隐私约束和实验评估等方面评估该项目的技术可行性。
评估表明，基于 Android 公开 API、Rust 核心链路、离线 replay 和受控动作边界，DiPECS 可以先实现一个可运行、可验证的端侧 AIOS 原型；高权限采集、真实系统预加载和复杂语义模型应作为后续阶段逐步推进。
后续工作需优先保证数据链路、隐私边界和动作治理正确，再逐步证明预测模型和真实动作是否带来性能收益。
...
即使 Android 真机能力尚未完全接入，系统仍可通过 replay 验证隐私、路由、策略和审计链路。
